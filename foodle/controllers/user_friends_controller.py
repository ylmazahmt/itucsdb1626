...
def remove(id):
	...
	next = request.form.get('next', "index")
	if next == "index":
		return redirect("users/"+ str(id) + "/friends/", code=302)
	elif next == "search":
		search_friend = request.form.get('search_friend', "")
		return redirect("users/"+ str(id) + "/friends/search/?search_friend="+search_friend, code=302)
...
def search(id):
	...
	string_to_search = request.args.get('search_friend', "")
	with psycopg2.connect(foodle.app.config['dsn']) as conn:
		with conn.cursor(cursor_factory=DictCursor) as curs:
			curs.execute(
			"""
